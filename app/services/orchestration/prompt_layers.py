...
def layer_formatting_constraints(*, query_type: str) -> str:
    ...
    return (
        ...
        f"STEP 1 — On first mention of a symptom, do NOT diagnose. "
        f"Acknowledge briefly, then ask ONE focused question (duration / "
        f"triggers / severity 1–10 / frequency). ONE only.\n"
        f"STEP 2 — Acknowledge each answer; ask ONE more if needed. "
        f"Never multiple per turn. Usually 2–3 exchanges.\n"
        f"STEP 3 — Once enough: 2–3 likely causes from the whole "
        f"conversation, practical today-actions, the doctor-confirmation "
        f"line, a clear next step.\n"
